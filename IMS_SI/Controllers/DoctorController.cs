...
namespace IMS_SI.Controllers
{
    public class DoctorController : Controller
    {
        ...
        public ActionResult ScheduleDetail()
        {
            string user = User.Identity.GetUserId();
            var doctor = db.Doctors.Single(c => c.ApplicationUserId == user);
            var schedule = db.Schedules.SingleOrDefault(c => c.DoctorId == doctor.Id);
            if (schedule == null)
            {
                return Content("<script language='javascript' type='text/javascript'>alert('No Schedule Available!');window.location.href = '/Doctor/Index';</script>");
